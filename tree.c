...
/* Until I get rid of this hack, make it linux/cygwin/HP nonstop only: */
  ...
  if (Hflag)
  {
    ...
    {
      if (force_color)
        fprintf(outfile, "<b class=\"NORM\">%s</b>", host);
      else
    ...
    {
      if (force_color)
        fprintf(outfile, "<a class=\"NORM\" href=\"%s\">%s</a>", host, host);
      else
        fprintf(outfile, "<a href=\"%s\">%s</a>", host, host);
    }
    curdir = gnu_getcwd();
  }
  else if (Xflag)
  ...
  if (dirname)
  {
    for (colored = i = 0; dirname[i]; i++, colored = 0)
    {
      ...
      if (Xflag || Jflag)
      {
        ...
        printit(dirname[i]);
      if (colored)
        fprintf(outfile, "%s", endcode);
      if (!Hflag)
        size += listdir(dirname[i], &dtotal, &ftotal, 0, 0);
      else
      {
        if (chdir(dirname[i]))
        {
  ...
  else
  {
    ...
    if (Xflag)
      fprintf(outfile, "%s<directory name=\".\">", noindent ? "" : "  ");
    else if (Jflag)
      fprintf(outfile, "{\"type\":\"directory\",\"name\": \".\",\"contents\":[");
    else if (!Hflag)
      fprintf(outfile, ".");
    if (colored)
      fprintf(outfile, "%s", endcode);
    size += listdir(".", &dtotal, &ftotal, 0, 0);
    if (Xflag)
      fprintf(outfile, "%s</directory>%s", noindent ? "" : "  ", _nl);
    if (Jflag)
      fprintf(outfile, "%s]}", noindent ? "" : "  ");
  }
  ...
  if (!noreport)
  {
    if (Xflag)
    {
      ...
      if (duflag)
        fprintf(outfile, "%s<size>%lld</size>%s", noindent ? "" : "    ", (long long int)size, _nl);
      fprintf(outfile, "%s<directories>%d</directories>%s", noindent ? "" : "    ", dtotal, _nl);
      if (!dflag)
        fprintf(outfile, "%s<files>%d</files>%s", noindent ? "" : "    ", ftotal, _nl);
      fprintf(outfile, "%s</report>%s", noindent ? "" : "  ", _nl);
    }
    else if (Jflag)
    {
      ...
      if (duflag)
        fprintf(outfile, ",\"size\":%lld", (long long int)size);
      fprintf(outfile, ",\"directories\":%d", dtotal);
      if (!dflag)
        fprintf(outfile, ",\"files\":%d", ftotal);
      fprintf(outfile, "}");
    }
    else
    {
  ...
  *n = 1;
  ...
  while ((ent = (struct dirent *)readdir(d)))
  {
    ...
    if (strlen(dir) + strlen(ent->d_name) + 2 > pathsize)
      path = xrealloc(path, pathsize = (strlen(dir) + strlen(ent->d_name) + 4096));
    ...
    {
      if (pattern && patmatch(ent->d_name, pattern) != 1)
        continue;
    }
    if (ipattern && patmatch(ent->d_name, ipattern) == 1)
      continue;
#endif
...
//    if (pattern && ((lst.st_mode & S_IFMT) == S_IFLNK) && !lflag) continue;
    ...
    if (p == (ne - 1))
      dl = (struct _info **)xrealloc(dl, sizeof(struct _info *) * (ne += MINC));
...
int psize(char *buf, off_t size)
{
  ...
  if (hflag || siflag)
  {
    ...
    if (!idx)
      ...
      return sprintf(buf, ((size / usize) >= 10) ? " %3.0f%c" : " %3.1f%c", (float)size / (float)usize, unit[idx]);
  }
  else
    return sprintf(buf, sizeof(off_t) == sizeof(long long) ? " %11lld" : " %9lld", (long long int)size);
}
